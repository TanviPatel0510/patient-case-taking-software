...
export function MedicalDocsButton() {
  ...
  return <div className="mt-8 border-t border-[#d1e2dc] pt-6"><div className="flex items-center justify-between gap-4"><div><h3 className="text-lg font-bold text-[#142a30]">Medical history</h3><p className="mt-1 text-xs text-[#556e72]">Keep prescriptions, reports, and summaries together.</p></div><Button type="button" className="inline-flex items-center gap-2 rounded-full bg-[#0c5e5b] px-4 py-2.5 text-xs font-bold text-white hover:bg-[#084341] cursor-pointer" onClick={() => setOpen(true)}><RiAddLine className="size-4" /> Add Medical Docs</Button></div>{error && <p className="mt-3 text-xs font-medium text-red-700">{error}</p>}<div className="mt-4 grid gap-3">{bundles.map((bundle) => <MedicalHistoryBundleCard key={bundle._id} bundle={bundle} onDelete={removeDocument} />)}</div>{open && <MedicalBundleForm onClose={() => setOpen(false)} onCreated={(bundle) => { setBundles((current) => [bundle, ...current]); setOpen(false); }} />}</div>;
}